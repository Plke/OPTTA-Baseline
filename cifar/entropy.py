...
def forward_and_adapt(x, optimizer, alpha, criterion, model0, model, nr, thresh_hold):
    ...
    open_ind = entropys[entropys >= thresh_hold]
    close_ind = entropys[entropys < thresh_hold]
    # print("open_ind", open_ind)
    ...
    loss = 0
    if len(close_ind) != 0:
        loss = close_ind.mean(0)
    if len(open_ind) != 0:
        loss -= alpha[1] * open_ind.mean(0)
